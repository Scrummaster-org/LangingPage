...
const Features = () => {
  ...
  return (
    <section id="services" className="py-12 sm:py-16 lg:py-20 text-white overflow-hidden relative" ref={ref}>
      {/* Dark overlay */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div
          ...
          className="space-y-12 sm:space-y-16 lg:space-y-24" // Reduced spacing on mobile
        >
          {/* Section Header - Mobile optimized typography */}
          <motion.div variants={slideInLeft} className="text-center space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white" // Progressive text scaling
                style={{
                  textShadow: '0 0 20px rgba(34, 197, 94, 0.6), 0 0 40px rgba(34, 197, 94, 0.3)'
                }}>
              Why Choose Our Bootcamp?
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-300 max-w-4xl mx-auto leading-relaxed px-2 sm:px-0"> {/* Better mobile text size */}
          ...
          {/* Alternating Features - Stack on mobile, side-by-side on desktop */}
          {features.map((feature, index) => (
            ...
            >
              ...
              <motion.div
                ...
                className={`space-y-6 sm:space-y-8 order-2 lg:order-none ${index % 2 === 1 ? 'lg:col-start-2' : ''}`} // Image first on mobile
              >
                {/* Icon + Title - Mobile responsive layout */}
                <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-4 mb-6">
                  <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-green-400/20 to-green-600/20 border border-green-400/40 rounded-2xl flex items-center justify-center flex-shrink-0" // Smaller on mobile
                       style={{
                  ...
                  <div className="text-center sm:text-left">
                    <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2"> {/* Responsive title sizing */}
                      {feature.title}
                    </h3>
                  </div>
                </div>
                ...
                {/* Benefits - Stack on small mobile, 2 cols on larger screens */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {feature.benefits.map((benefit, idx) => (
                    <div key={idx} className="flex items-center space-x-3 p-3 bg-gray-900/30 rounded-lg border border-gray-800/50">
                      ...
                      <span className="text-gray-300 text-xs sm:text-sm font-medium">{benefit}</span>
                    </div>
                  ))}
                </div>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="w-full sm:w-auto bg-green-400 text-black font-bold py-4 px-6 sm:px-8 rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-green-400/25 min-h-[48px]" // Full width on mobile, min touch target
                >
                  <span className="text-sm sm:text-base">Learn More About {feature.title}</span>
                </motion.button>
              </motion.div>
              ...
              <motion.div
                ...
                className={`relative order-1 lg:order-none ${index % 2 === 1 ? 'lg:col-start-1' : ''}`}
              >
                <div className="relative aspect-[4/3] sm:aspect-[16/10] lg:aspect-[16/10] bg-white rounded-xl sm:rounded-2xl border border-gray-300 overflow-hidden backdrop-blur-sm"> {/* Better mobile aspect ratio */}
                  <Img
                    src={feature.imageSrc}
                    alt={feature.imageAlt}
                    className="absolute inset-0 w-full h-full object-contain rounded-xl sm:rounded-2xl"
                  />
          ...
          {/* Bottom Stats Section - Mobile optimized */}
          <motion.div
            variants={slideInLeft}
            className="relative mt-16 sm:mt-24 lg:mt-32" // Responsive top margin
          >
            ...
            {/* Stats container with mobile padding */}
            <div className="relative bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl sm:rounded-2xl p-6 sm:p-8 lg:p-12">
              <div className="text-center mb-8 sm:mb-12">
                <h3 className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold text-white mb-4" // Progressive heading sizing
                    style={{
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
                {stats.map((stat) => (
                  ...
                  >
                    ...
                    <div className="relative w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-green-400/30 to-green-600/20 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 group-hover:scale-110 transition-all duration-500"
                         style={{
                    ...
                    {/* Content with mobile typography */}
                    ...
                    <div className="absolute top-2 right-2 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-400/60 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  </motion.div>
                ))}
              </div>
